...
## 최신 TD-54 — DB 우선 단일 호출 경로

사용자가 대기시간·비용 개선을 위해 편집 후보 DB와 단일 선택/보충을 승인했다. [DB 우선 엔진 설계](docs/CATALOG_ENGINE.md)는 새 `catalog` 전략의 신뢰 수준·V4 seed·원문 조건 처리·0/1회 경로·실험 기준을 정의한다. 기존 `staged` 독립 검토 정책과 certificate는 변경하지 않는다. 공용 API·불변 snapshot·게임·일일/누적 비용 제한도 보존한다. 새 전략은 명시 설정으로 실험하며 운영 적용 완료는 별도로 검증한다.
...
## 8. 후보 재사용 — TD-42 구현 승인, 검증 진행

2026-09-18 사용자가 DB 재사용 검증 후 서비스 마무리·배포를 승인했다. 모델 파라미터를 학습하는 fine-tuning이 아니라 검증된 후보 지식을 축적하고 검색하는 구조다. 기존 draft는 24시간 뒤 정리되며 snapshot은 공유 재플레이용 불변 사본이다. 이를 재사용할 수 있는 승인된 후보 지식으로 자동 간주하지 않는다. 아래 단계 중 현재 구현 범위는 1단계이며, 2/3단계의 임의 조건 변경·조합은 아직 제공하지 않는다.

현재 구현: 기존 worker 앞에서 정확한 요청 전체/N/locale/timezone/정책 버전 해시로 승인 세트를 조회한다. 조건의 의미를 바꿀 수 있는 문장 유사도·내부 공백 삭제·키워드 매칭은 하지 않는다. history가 있는 요청은 공용 재사용에서 제외한다. 신규 engine 성공은 원래 검증 자료를 보존해 PENDING으로만 저장하며 stale worker는 저장할 수 없다. 명시적 운영자 품질/공용 안전성/시간 독립성 승인과 최대 7일 만료가 필요하다. 재사용할 때에도 원래 근거·feasibility/해석 판정과 gate를 다시 검사하며, 오래된 정책·만료·철회·동일 후보군 재생성을 거절한다. 원문/history/constraint sourceText를 공용 payload에 복사하지 않는다. 해시를 개인정보 익명화 보장으로 주장하지 않는다. 자동 READY나 AI 평가만으로 사람 승인 데이터를 만들지 않는다.
...
PostgreSQL 안의 별도 재사용 기록을 사용하며 새 vector DB·embedding·계층 분류 모델을 추가하지 않는다. cache hit 비율/검토 호출 비용/품질 회귀를 측정하기 전 절감률을 약속하지 않는다. 공개 배포 작업은 승인됐지만 사업자·운영 월 예산·실제 승인 데이터는 아직 미확정이다. reuse hit도 기존 일일 접수 한도에 포함하고 provider 호출 비용만 발생시키지 않는다.
