...
현재 실제 합성 입력 테스트에서는 **8강 READY/preview 연결만 성공**했습니다. 같은 입력의 16/32강은 Repair 이후에도 품질 미달로 FAILED였습니다. 모든 강수의 실서비스 품질이 준비된 상태가 아닙니다. A는 dev로 전체 강수의 화면·오류 흐름을 구현하고, 실제 AI 품질 확정은 B와 별도로 진행합니다.
