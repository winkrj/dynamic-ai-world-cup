...
v4 실제 합성 입력 테스트에서 **8강 READY/preview 연결만 성공**했습니다. 같은 입력의 16/32강은 Repair 이후에도 품질 미달로 FAILED였습니다. 최신 v6는 후보 활동을 먼저 검토하는 구조로 바뀌었으며, 실행한 8/16강은 승인 활동 부족으로 상세 생성 전에 실패했습니다. **최신 v6의 실제 READY 증거는 아직 없고**, 이전 v4 성공을 최신 버전의 성공으로 보지 않습니다. 공개 API는 그대로지만 모든 강수의 실서비스 품질이 준비된 상태는 아닙니다. A는 dev로 전체 강수의 화면·오류 흐름을 구현하고, 실제 AI 품질 확정은 B와 별도로 진행합니다.
