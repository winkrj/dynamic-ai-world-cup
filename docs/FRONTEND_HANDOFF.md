...
**이전 버전에는 8/16/32강 실제 READY·preview 연결 사례가 있지만 최신 품질 통과를 뜻하지 않습니다.** v22는 평범하거나 호불호 있는 적격 후보를 허용하되 필수 조건·중복·비교 단위·지속성·실행 가능성·필수 근거 검사를 유지합니다. 최신 실제 `hobby-solo/16` 검증은 재검토의 UNIT 해석 FAIL과 그림 매체 중복으로 활동 15/16만 승인돼 FAILED였으며 preview가 없습니다. 낮은 재미나 API 계약 오류가 원인은 아닙니다. 누적 장부는 $4.9355993/$6이고 자동 충전 OFF를 유지합니다. TD-50에 따라 서버 담당자의 목적이 명확한 실험은 순차 1건씩 사전 승인됐지만, 운영 트래픽·무한 재시도·예산 인상 승인은 아닙니다. 프론트는 생성 요청을 자동 반복하지 않습니다. 사전·상세 Repair는 합쳐서 최대 1회입니다.

**회귀 검증과 후보 품질을 구별합니다.** 최신 전체 회귀는 Java 327개 실행/유료 2개 제외, 프론트 60개, 실제 HTTP 166개/9 schemas, 웹·두 JAR가 통과했고 독립 리뷰 Critical/High/actionable 0입니다. 여기서 HTTP는 격리 서버와 합성 후보 검증입니다. 별도로 실행한 v22 실제 테스트 1개는 위 품질 실패로 FAIL입니다. 실제 READY 결과를 추가 AI 호출 없이 freeze→N−1 선택→Champion→동일 snapshot 공유까지 검사하는 경로는 이미 연결돼 있지만, 이번에는 READY에 도달하지 못해 실행되지 않았습니다. 저장·공유 기능을 다시 만들거나 fixture 후보를 실제 AI 결과로 표시하지 않습니다. 사람 평가의 비교 단위/실행 가능성 FAIL과 외부 사실 평가 미완료는 별도입니다.
