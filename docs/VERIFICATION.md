...
- 합성 복구 브라우저8사례×360/1280 PASS, 키보드/reduced motion/수정·새로고침·자동POST0/오류0. 360px 오류 안내 screenshot을 직접 확인했다. CSS/타이머/카드 변경 없음.
...
- 실제 모바일 Safari/스크린리더는 미검증이다. 기존 TD-55 운영 결과를 TD-56 배포 증거로 재사용하지 않으며 아래 별도 관측을 따른다.

TD-56 운영 후속: 소스 `521fbfe341264f0042f372ac06439be6c3b8a78c`, linux/amd64 digest `sha256:efb46fcd3c31078bca86b318f96bb17dfa3298b38904c10223f4982c75b6619f`의 앱만 교체했다. 실제 catalog/30/prod,live,proxy/누적5·healthy, HTTPS root/sharedeep-link/asset2/health/ready/404 PASS. 공개 `index-Cvf931Db.css`와 `index-XL7YxgIu.js`는 로컬 최종 검증 파일과 바이트 단위로 같다. 기존snapshot5개의 전체 집계 MD5 `505c0ea279c67af33e2f6196ab091e9c`, 카탈로그64, 운영provider14건/$0.73137410/예약0는 전후 동일하다. 이번 작업의 유료 호출은0이며 TD-55 이후 증가분을 이번 테스트 비용으로 분류하지 않는다.

실제 Chrome의 기존공유16강에서 강수 경로·마지막3초·8강/준결승/결승·Champion저장을 확인했다. 직접7/timeout8과 DB COMPLETED/sequence15가 일치하고 console warn/error0이다. 실제 타이머가 대진을 변경해 테스트 제어의 특정 이름 클릭/완료 후 클릭이 만료된 경우는 현재 화면을 다시 읽었으며 제품 오류로 분류하지 않았다. 공개 화면 관측을 정확한500ms/7초 벽시계 측정으로 주장하지 않는다. 정확한 gate/deadline 근거는 앞의 합성 브라우저 테스트다. 백업69,205bytes/AES256/version와 timer active 확인, 새 백업의 격리 복원은 미실행이다. 배포 첫 명령은 백업 뒤 후속 줄이 실행되지 않아 실제 이미지를 재확인하고 stdin과 분리한 명령으로 교체 단계만 진행했다. 성공 상태 문자열만으로 배포 완료를 판정하지 않았다.

배포 후 후속 커밋은 위 실제 관측의 사실 기록만 변경한다. 런타임 동작 위험이 없는 문서 변경이므로 추가 코드 Reviewer·전체 테스트는 생략하고 `git diff --check`를 실행했다. 코드의 독립 리뷰는 앞의 1차 결과다.

이전 2026-09-20 TD-55: 제출본 `fcb6a5f`의 전체 verify·독립리뷰(Critical0/High0)·합성 브라우저8/16/32와 **AWS 배포·실제 운영 연결 검증 완료**. Java452실행/유료2제외·프론트61·배포정적32는 같은 소스의 배포 전 결과다. [최신 제출 현황](V1_SUBMISSION.md)을 따르며 아래 운영 기록과 과거 미배포/v2/숫자는 당시 기록이다.

## TD-55 공개 운영 검증 — 2026-09-20

동일 검증 이미지 `sha256:6bcf312d9520d3fb0000f73215afa2b3a768b6c30b82966307ff42f252408809`를 기존 AWS에 설치했다. 실제 catalog/30/prod,live,proxy/누적5, V4/seed64, HTTPS root·sharedeep-link·asset2·health/ready·없는API/asset404 PASS. 배포 전후 기존snapshot MD5가 같고, 기존 공개공유200·비용장부 보존·백업timer active다.

| 경로 | 실제 결과 |
| --- | --- |
| DB preset16 | HTTP접수→preview767ms, provider0, catalog/hobby-basic |
| 조건부 취미16/v3 | preview5357ms, provider4455ms/COMPOSE1/검색0/입력4271·출력144, 비용$0.012404, READY, API15경기/Champion 저장 |
| 초기 점검기 공유 | FAIL: 빈 본문 계약에 임시 도구가 `{}`를 전송해400. 제품 코드 변경 없음. 초기 실패 증거 보존 |
| 후속 DB전용 확인 | 새 DBpreset preview613ms/provider0. 올바른 빈 본문으로16강완주→공유→새익명session의snapshot deep equality PASS. 앞의 AI생성은 재실행하지 않음 |
| 공개 브라우저 | DB공유16강→15회TIMEOUT_RANDOM→Champion저장→새공유링크. DB의COMPLETED/sequence15/events15와일치, console warn/error0. 직접 선택 또는 정확한 벽시계 간격을 검증한 결과로 세지 않음 |
| 최종 예산 | 운영provider11건/$0.697184/$5, RESERVED0, 공유와브라우저플레이 뒤 추가호출0. 실험$5.4206158/$6은변경없음 |

임시 도구는 이번 운영 점검에만 사용했다. 로컬 합성 전용 smoke의 운영 차단을 우회하거나 제거하지 않았다. 최초HTTP 점검기 실패는 후보품질 FAIL 또는 제품공유기능 결함으로 바꾸지 않으며, 후속DB·실제브라우저 성공으로 최초실패 기록을 덮지 않는다. 이번 제출 후속은 배포 및 사실 기록만 바꾸었으므로 Java/프론트 전체 테스트와 코드 Reviewer를 새로 실행했다고 주장하지 않는다. 기존 독립 검토를 통과한 소스는 그대로다.

내용 관측: 실제16개는 독서·그림그리기·베이킹·외국어학습·뜨개질·디지털작곡·실내식물·창작글쓰기·마술·코딩·영상편집·레고·핸드드립·직소퍼즐·종이접기·명상이다. 한 사례의 속도/비용/구조 성공이지 모든 조건의 의미검증 또는 사람품질승인이 아니다. 과거32강 유사성 보류는 유지한다. 실제백업S3저장/암호화는확인했지만 이번V4격리복원은미실행이다.
