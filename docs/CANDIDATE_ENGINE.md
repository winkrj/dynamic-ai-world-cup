...
공개 계약/DB/schema·모델·호출 수·Repair 합계/예산은 그대로다. 지침 전달과 합성 gate 테스트는 실제 모델 판단 개선을 증명하지 않는다. v18에는 승인된 활동도 29개뿐이어서 표현 문제가 해결돼도 중복·장비 접근 및 후보 수 확보 검증은 남는다. v19 유료 호출은 실행하지 않았으며 추가 승인도 없다. [OpenAI 프롬프트 지침](https://developers.openai.com/api/docs/guides/prompt-engineering)의 명확한 역할·예시와 [평가 지침](https://developers.openai.com/api/docs/guides/evaluation-best-practices)의 실제 결과·사람 판단 대조를 구별한다.
