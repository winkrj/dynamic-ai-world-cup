...
공개 API/DB/schema·모델·호출 수·Repair 합계 1회·비용 예약은 변경하지 않는다. 하모니카는 일반 원칙의 대조 예시이지 무조건 승인하는 후보가 아니다. 과거 v17 실행은 당시 응답/기준의 실패 기록으로 보존하며, 이번 사용자 판단으로 해당 실행의 preview를 소급 발급하지 않는다. v18 실제 판단 정확도·후보 다양성·비용·latency는 아직 미검증이고 추가 유료 실행 승인도 없다.
