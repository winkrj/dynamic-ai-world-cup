...
최신 2026-09-19 TD-54: 사용자 요청으로 DB 편집 후보 + 최대 1회 compact 선택/보충하는 별도 catalog 전략을 구현·실험했다. best-effort 의미 품질을 독립 검증과 구별하며 아래 staged 코드는 보존한다. [새 설계·실제 결과](CATALOG_ENGINE.md), [검증](VERIFICATION.md)을 따른다. 운영은 아직 staged이며 아래 기록을 새 전략의 성공률/예산으로 재해석하지 않는다.

2026-09-18 갱신. B(후보 품질과 서버) 소유. AC-02~07/14/18이 대상이며 공개 OpenAPI와 프론트 소유 경로는 변경하지 않는다. 이 문서는 구현 설계이며 실제 검증 범위는 버전별로 구별한다.

## TD-51 / v23 — 생성·독립 검토·Repair로 호출 구조 단순화
...
사용자가 **호출 구조 단순화**를 명시 승인했다. 조건/선호→비교 단위→관련 직접 선택 이력→coverage→후보를 구조화 생성에서 순서대로 작성하고, 생성 뒤 독립 검토에서 전체 세트를 평가한다. 외부 사실이 필요한 경우에는 이 검토 전에 기존 selective grounding이 필요하며 검색 승인 없이는 실행하지 않는다. 필수 판정 FAIL/UNKNOWN이면 미리보기를 내보내지 않고 기존 Repair 최대 1회와 재검증만 허용한다. 사전 의도 검토·의도 Repair 호출을 없애는 것이지 독립 검토나 중복 차단을 없애는 변경이 아니다.
...
TD-51은 TD-50과 별개로 받은 설계 전환 승인이다. 프롬프트는 `ce002-v23-generate-review-repair`, 재사용 정책은 `approved-complete-set-v4-engine-v23`이다. 구현과 관련 262개·독립 리뷰 Critical/High/actionable 0·전체 verify(Java 358개 실행/유료 2개 제외·프론트 60개·HTTP 166개/9 schemas·웹/두 JAR)는 PASS다. 이 설계 변경을 과거 FAIL 소급 승인이나 이미 중단된 문구 수정 루프의 재개로 간주하지 않는다.

### 구현 경계

| 단계 | 책임 / 실패 처리 |
| --- | --- |
| GENERATE | 하나의 strict JSON 응답에 plan을 먼저, candidates를 뒤에 작성한다. 해석·quota 자체가 잘못되거나 전체 JSON이 손상되면 신뢰할 고정 계획이 없으므로 종료한다 |
| 고정 계획 / 구조 검사 | 요청 N, 출처가 있는 필수 조건, 단위, coverage와 quota 합을 검사한다. 신뢰 가능한 계획 아래 후보 구조 결함은 Repair 대상이다. 여기의 계획 고정은 게임 시작 시 bracket freeze와 별개다 |
| GROUND → REVIEW | 필요한 외부 사실만 조회한 뒤 별도 요청에서 원문과 계획의 해석·전체 후보의 비교 가능성/중복/적격성/feasibility·각 조건을 판단한다. 생성자의 자기 판정은 없다 |
| REPAIR → 전체 재검토 | 후보 문제만 최대 1회 수정한다. 지적된 ID의 핵심 활동 교체는 허용하되 계획/조건/단위/coverage와 지적되지 않은 후보는 보존한다. 필요 근거를 다시 확인하고 전체 세트를 새로 검토한다. 해석 FAIL/UNKNOWN은 후보 교체로 숨기지 않고 종료한다 |
| 노출 / DB 재사용 | 실제 최종 검토와 deterministic gate를 모두 통과한 결과만 preview를 발급한다. 최종 원본 증거만 certificate에 저장한다. 구 allocation 필드는 제거하며 가짜 PASS를 만들지 않는다. 구 정책/증명은 재사용 miss, 기존 snapshot/display copy와 공유는 불변이다 |

검색 없는 정상 경로는 GENERATE·REVIEW의 2개 provider 요청이며 수정이 필요하면 REPAIR·REVIEW가 더해져 최대 4개다. 한 실험과 내부 요청 수를 구분해서 비용을 합산한다. 8/16/32, 모델, 8192 출력 한도, deadline, 호출별 예약·누적 예산, 공개 HTTP 계약/프론트는 바꾸지 않는다. 특히 32강은 응답 길이와 출력 잘림을 실제 관측해야 하며 16강 성공만으로 32강 품질을 주장하지 않는다. 과거 고정 v21 진단용 allocation adapter는 runtime interface 밖에 남기되 소진한 실험을 재사용하지 않는다.

### 실제 v23 / 기본 16강 연결 1회

TD-50/51 승인 범위에서 원문 그대로 `seed-v1/hobby-solo/16`을 검색 없이 1회 실행했다. 결과 **READY 및 LiveEngineHttpTest 1개 PASS**다. Terra 4회 HTTP 200, Repair 1회, 전체 127.012초, 사용량 추정 **$0.1380685**다. GENERATE 58.773초/$0.039508 → REVIEW_INITIAL 14.883초/$0.028126 → REPAIR 31.208초/$0.038982 → REVIEW_REPAIRED 21.441초/$0.0314525였다.

첫 검토는 조건·비교 가능성·중복·candidateQuality를 PASS했으나 c13 ‘테마 요리’의 주방 접근이 미확인이라며 feasibility UNKNOWN을 반환했다. 서버는 aggregate PASS로 이 판정을 덮지 않고 c13만 한 번 교체했다. ‘혼자 하는 보컬 연습’으로 바뀌었고 나머지 15개 후보의 모든 필드와 계획은 동일했다. 최종 해석·비교·중복·quality PASS, 조건 32개 PASS, feasibility 16개 PASS, findings 0을 확인했다. 이것은 일반적인 주방 접근까지 확인받아야 한다는 새 제품 규칙이 아니다. TD-44의 통상 준비/외부 접근 구분을 모델이 과도하게 적용한 관측으로 남긴다.

실제 preview를 그대로 시작해 15경기 telemetry를 저장하고 Champion, 같은 immutable snapshot 공유, 다른 익명 사용자의 새 세션까지 검사했다. 이 후속 흐름은 생성 작업 1개와 기존 4개 provider 요청을 유지해 추가 AI 호출 0회다. 실제 응답 14개/8 schemas를 공개 OpenAPI로 재검사했다. 서버 HTTP 연결 검증이며 사용자가 브라우저에서 실제 7초를 기다려 플레이한 실험은 아니다.

비공개 증거: `reports/local/live-engine/2026-09-18T14-13-08.608264Z-size16/`의 run/job/preview/playback/exchanges/ledger/http-samples. 모든 요청 tools=[]·search_calls=0, 4개 장부 COMPLETED다. 누적 provider 시도 152회, 사용량 추정 $4.5736678 + 이전 미확인 예약 $0.50 = **장부 $5.0736678/$6**, 잔여 $0.9263322다. 실행 전 UI의 자동 충전 OFF를 확인했고 설정은 변경하지 않았다. 기본 유료 플래그는 false이며 재시도하지 않았다.

한 사례의 기술 연결 성공은 모든 입력의 품질·성공률·속도·비용 개선율을 입증하지 않는다. v23의 8/32강 실제 실측, 외부 사실 검색 정확도, 기존 R 사람 평가 FAIL/두 사람 독립 평가, 공개 배포는 별도 남은 영역이다. 18세트 첫 실행 현황도 변경하지 않는다. 연결은 완료됐지만 이 결과를 공용 재사용 데이터로 자동 승인하거나 운영 트래픽을 켜지 않았다.

### 실제 v23 / 32강 출력·시간 경계 첫 관측

같은 코드 `fc35557`에서 TD-50/51에 따라 `seed-v1/hobby-solo/32`를 검색 없이 한 번 실행했다. 목적은 합쳐진 32강 응답의 길이·시간 경계와 가능한 경우 같은 결과의 저장/공유를 관측하는 것이다. 프롬프트·모델·상한 변경이나 이전 실패를 성공할 때까지 재추첨하는 실험이 아니다.

결과는 **FAILED / QUALITY_GATE_FAILED**, 실제 통합 테스트 1개 FAIL/Gradle 종료 1이다. 185.026초/4회 HTTP 200/Repair 1회/검색 0, 사용량 추정 **$0.221852**다. GENERATE 66.921초/$0.052425 → REVIEW_INITIAL 40.556초/$0.054152 → REPAIR 34.917초/$0.0594275 → REVIEW_REPAIRED 42.147초/$0.0558475였다. 출력 토큰은 순서대로 3,782/2,968/3,612/3,102이고 모두 completed다. 이 사례에서는 8,192 출력 한도·90초 요청·280초 엔진 제한에 걸리지 않았지만 일반적인 32강 성능 보장은 아니다.

첫 검토는 해석·비교·중복·quality와 조건 64개를 PASS했으나 c30 ‘식물 관찰 기록’의 집 주변 식물 접근을 UNKNOWN으로 봤다. Repair는 c30만 ‘향신료 블렌딩’으로 교체했다. 나머지 31개 카드의 모든 필드가 동일한 것을 원시 응답으로 대조했다. 전체 재검토에서는 조건 64개·feasibility 32개·해석/비교/quality PASS였지만, 그대로인 c7 ‘연필 드로잉’·c8 ‘색연필 컬러링’·c9 ‘수채화’를 그림 그리기의 재료/기법 변형으로 중복 FAIL 처리했다. 그림을 쪼개 개수를 채우지 말라는 기존 사람 피드백과 관련된 결함이지 낮은 매력만의 문제가 아니다. 첫 검토가 이를 놓친 판정 불일치도 남았다.

서버는 Repair를 두 번 쓰거나 FAIL을 PASS로 바꾸지 않고 종료했다. preview가 없어서 31경기 저장·공유 후속 검사는 실행되지 않았다. 실제 QUEUED/FAILED 응답 2개는 GenerationJob 계약 검사 PASS다. 첫 검토의 누락을 최종 차단 성공으로 덮지 않으며 이 실패를 사람 품질 승인이나 32강 지원 삭제·자동 축소 결정으로 바꾸지 않는다. 코드/프롬프트 수정·추가 호출은 하지 않았다.

비공개 근거는 `reports/local/live-engine/2026-09-18T14-32-55.614295Z-size32/`다. 모든 요청 tools=[]·search_calls=0, 장부 4건 COMPLETED를 대조했다. 누적 provider 시도 156회, 사용량 추정 $4.7955198 + 이전 미확인 예약 $0.50 = **장부 $5.2955198/$6**, 잔여 $0.7044802다. 실행 전 결제 UI에서 자동 충전 OFF를 확인했고 변경하지 않았다. 선불 화면 잔액은 장부를 대체하지 않는다. 최신 8강 실측·외부 사실/사람 평가·운영 배포는 별도이며, 위 16강 PASS와 과거 실패 기록을 보존한다.
