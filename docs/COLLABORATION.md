...
> 2026-09-18 현재 결정: 사용자가 프론트도 맡아 동일 작업에서 A/B를 병렬 개발한다. 외부 팀원 인계는 더 이상 선행 조건이 아니다. 현재 통합 브랜치는 `feat/product/split-deck-flow`이며 PM·디자인·프론트 상태·백엔드의 파일 경계/비판적 검토 규칙은 루트 `AGENTS.md`가 기준이다. 아래 두 사람의 clone/PR 안내는 별도 팀원이 합류할 때의 절차로 보존한다. 현재 엔진 수정·유료 실험은 보류한다.
