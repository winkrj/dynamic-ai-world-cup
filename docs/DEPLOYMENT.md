...
사용자는 서비스 완성과 배포 진행을 요청했다. 실제 사업자·계정·지역·도메인, 인프라 월 비용 상한과 AI 운영 예산은 아직 정하지 않았다. 이 문서는 특정 사업자나 새 유료 자원을 선택하지 않는 실행 준비다. 기존 실험 예산을 공개 서비스의 반복 AI 호출 예산으로 해석하지 않는다.
