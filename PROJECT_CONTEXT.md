...
- 통합 JAR/비루트 컨테이너, DB·웹 번들·엔진 설정 readiness, prod 오설정 시작 차단, 읽기 전용 release 점검을 준비 중이다. 실제 배포 사업자/월 운영비/도메인·프록시 선택은 답변 대기다. 신규 과금 자원은 만들지 않았다.
- 최종 verify 통과: Java 309개 실행/유료 1개 제외, 프론트 60개, handoff 6개, release 13개, fixture 5개, HTTP 160개/9 schemas, 웹/두 JAR. 독립 Reviewer Critical/High/actionable 0. Docker 웹 stage만 통과했고 전체 runtime 이미지는 저장소 I/O 오류로 실패(맥 여유 161MiB); 임의 정리 없이 후속 실행을 보류했다. 상세는 `docs/VERIFICATION.md`다. 실제 AI 품질·공용 데이터 승인·최신 실제 AI와 프론트 완주·공개 운영 배포는 미완료다. 미리보기는 현재 계약대로 이름/태그이며 준비물 상세 표시의 충분성은 여전히 사람 평가 대상이다.
