...
test('clarification and draft answers including over-limit edits survive reload without truncation', () => {
  const values: Flow[] = [
    ...
    { kind: 'generation', input, key: 'initial-rate-limit', manualRetry: true, rejection: 'RATE_LIMITED' },
    { kind: 'generation', input, key: 'grounding', jobId: 'failed', failed: true, failedCode: 'GROUNDING_REQUIRED' },
  ];
  for (const flow of values) assert.deepEqual(restoreFlow(encodeFlow(flow, 1000), 1500), { flow });
});
...
test('invalid clarification metadata or generated request length is rejected safely', () => {
  for (const flow of [
    ...
    { kind: 'generation', input, key: 'key', rejection: 'RATE_LIMITED' },
    { kind: 'generation', input, key: 'key', manualRetry: true, rejection: 'UNKNOWN' },
    { kind: 'generation', input, key: 'key', jobId: 'accepted', manualRetry: true, rejection: 'RATE_LIMITED' },
    { kind: 'generation', input, key: 'key', failedCode: 'GROUNDING_REQUIRED' },
