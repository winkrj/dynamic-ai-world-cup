...
| AC-27 | 비교 단위를 취미로 제한하지 않음. 작품 요청은 개별 작품을 후보로, 동일 장르/가수의 다른 작품은 별도 선택으로 지시. 동일 작품 별칭/재발매는 동일 family, 알려진 중복/N/단위 검사는 유지. 무관한 catalog는 선택하지 않고 additions로 보충 가능. 최신 순위·현재 이용 가능성을 일반 작품으로 몰래 바꾸지 않음. 지침/합성 테스트 통과와 실제 제목 정확도/모델 관측을 구분 |
...
| AC-19 | 기본은 익명 브라우저별 하루2회/서울 자정. 명시 설정0은 심사용 일일 cap만 해제하며 음수 거부·누락 기본2. cap0에서도 접수 기록·동시성 lock·actor/IP5회/10분·비용 상한 유지. 접수 후 FAILED 포함, 동일 key 재전송·접수 rollback·Repair·공유 플레이 추가 차감 없음. 제한 복구 시 기존 기록을 보존하고 정리 작업이 당일 기록을 지우지 않음. 429에 실제 해제 시각까지의 Retry-After와 수동 재시도 안내 | TD-40/57; 일반·해제 DB/HTTP·프론트 회귀 테스트 |
