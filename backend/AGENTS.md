...
TD-38/v18: 소음 제한이 없으면 일반적인 집 안 하모니카 연습을 제안할 수 있다는 사용자 판단을 반영한다. 통상적 집 안 환경과 특정 자연환경·전용 시설·특수 장비를 공통 생성/검토 프롬프트에서 구분한다. 명시 조건·grounding·UNKNOWN/FAIL 차단·Repair 합계 1회는 그대로다. 실제 유료 v18 평가는 별도 승인 전 실행하지 않는다.
...
후속 TD-37의 키 교체 후 v17 `hobby-solo/32` 추가 시도 1회도 사용 완료했다. 인증은 복구됐으나 하모니카 연습 환경 feasibility UNKNOWN으로 `QUALITY_GATE_FAILED`; 사전 Repair 1회/검색 0회/preview 없음. 현재 장부 $4.4511778, 잔여 $0.5488222이며 위 $0.50 예약·누적 $5·검색 없음·자동 충전 OFF를 유지한다. 새 승인 전 유료 재시도 금지. 실제 근거는 docs/CANDIDATE_ENGINE.md의 TD-37 결과를 따른다.
