...
현재 장부 $5.0736678/$6(미확인 예약 $0.50 포함), 잔여 $0.9263322이다. v23 hobby-solo/16 실제 1회는 Repair 1회 뒤 READY·15경기 저장·동일 snapshot 공유/새 세션까지 PASS, 127.012초/4회/$0.1380685/검색 0이었다. Java 358개·프론트 60개·전체 verify·독립 리뷰 Critical/High 0도 PASS다. 이는 모든 입력/32강의 품질, 사람 승인, 운영 배포 완료가 아니다. 매 실험 전 전체 장부를 다시 합산하고 격리 DB의 새 예산을 누적 상한으로 착각하지 않는다. 선불 잔액 차단에는 처리 지연이 있을 수 있으므로 잔액 소진에만 의존하지 않고 기존 서버 예약/상한을 유지한다.
