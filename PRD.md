...
현재 제출 정책(TD-58, 2026-09-20): 운영은 `catalog` 전략이다. 정확한 DB preset은 AI 0회, 일반 요청은 최대 1회이며 자동 검색·독립 review·Repair는 없다. 아래 staged의 엄격한 의미 gate/Repair 설명은 보존된 이전 전략이고 catalog에 수행된 것으로 주장하지 않는다. 지역·요일이 포함된 일반 활동 아이디어는 지원하며, 실명 장소·최신 영업/가격 확인은 미지원이다. 심사 중 일일 cap은0, 단기 actor/IP5회/10분·운영 누적$5는 유지한다. 사진·이모지 추가 없이 A 상하 카드와 현재 연출로 제출한다. 최신 검증·배포 결과는 [제출본](docs/V1_SUBMISSION.md)을 따른다.
